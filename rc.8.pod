...
B<rc.modules> is a command script to which local
kernel module initialization actions can be added.  It is (nearly) the
last thing invoked by B<rc> during system boot.

B<rc.local> is a command script to which local boot-time actions can
