# rc.conf.5.pod - documentation for rc.conf(5) manual page
...
This file is a simple POSIX L<sh(1p)> script that defines font,
timezone, hostname and services which are used on system initialization
and shutdown.
...
=item B<BLANKTIME>="0-60|force|poke"

Specifies the interval of console inactivity, in minutes, after which
the screen will be automatically blanked.  The contents of this
variable will be passed as argument to L<setterm(8)>.

The B<force> argument keeps the screen blank even if a key is pressed.

The B<poke> argument unblanks the screen.

If B<BLANKTIME> is unset, then L<rc(8)> make screen blanks after 15
minutes of idle time.

Example: C<BLANKTIME=15>

=item B<FONT>="I<STRING>"

Specifies which console font to load at system startup.  The contents
of this variable will be passed as argument to L<setfont(8)>.
...
contents of this variable will be passed as argument to L<loadkeys(1)>.
...
=item B<SEEDRNG_SKIP_CREDIT>="1|true|yes|y"

Specifies L<seedrng(8)> that seeds never credit the RNG, even if the
seed file is creditable.  Disabled by default.

Example: C<SEEDRNG_SKIP_CREDIT=false>
...
B<SERVICES> variable is as follows: firstly, the syslog daemon also
